...
const QRUtil = {
  ...
  getPatternPosition(typeNumber: number): number[] | undefined {
  ...
  getLostPoint(qrCode: QRCodeModelImpl): number {
...
type ResolvedOptions = Required<QRCodeOptions>;

function _getErrorCorrectLevel(ecl: string): number {
  switch (ecl) {
    case "L":
...
function _getTypeNumber(content: string, ecl: string): number {
...
class QRCode {
  ...
  constructor(options: QRCodeOptions | string) {
    ...
    model.make();
    this.qrcode = model as unknown as QRCodeModel;
  }
  ...
  svg(opt?: SvgOptions): string {
    ...
    const bgrect = `${indent}<rect x="0" y="0" width="${width}" height="${height}" style="${qrSvgStyle(options.background)}"/>${EOL}`;
    ...
    for (let y = 0; y < length; y++) {
      for (let x = 0; x < length; x++) {
        ...
        if (module) {
          ...
          if (options.join) {
            let w: number | string = xsize + (px as number);
            let h: number | string = ysize + (py as number);

            px = Number.isInteger(px) ? Number(px) : (px as number).toFixed(2);
            py = Number.isInteger(py) ? Number(py) : (py as number).toFixed(2);
            w = Number.isInteger(w) ? Number(w) : (w as number).toFixed(2);
            h = Number.isInteger(h) ? Number(h) : (h as number).toFixed(2);

            pathdata += `M${px},${py} V${h} H${w} V${py} H${px} Z `;
          } else {
            modrect += `${indent}<rect x="${px}" y="${py}" width="${xsize}" height="${ysize}" style="${qrSvgStyle(options.color)}"/>${EOL}`;
          }
        }
      }
    }
    ...
    if (options.join) {
      modrect = `${indent}<path x="0" y="0" style="${qrSvgStyle(options.color)}" d="${pathdata}" />`;
    }
...
export function qrSvgStyle(color: string): string {
  return `fill:${color};shape-rendering:crispEdges;`;
}
