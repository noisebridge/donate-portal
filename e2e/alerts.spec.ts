...
test.describe.serial("Alerts Page Tests", () => {
  test.use({
    httpCredentials: {
  ...
  test("latest donation shows correct amount and product name", async ({
    ...
    await expect(page.locator("#alert-product")).toHaveText("General Donation");
  });
  ...
  test("$10 donation appears in history with correct name", async ({
    ...
    await expect(
      historyItem.first().locator(".history-product"),
    ).toHaveText("General Donation");
  });
